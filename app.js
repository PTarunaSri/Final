...
//SUBMIT MODULE
app.post('/visitor/submit', async function(req, res){
    try
    {
        conn = await client.connect();
        db = conn.db('MSWD');
...
app.post('/myprofile/info', async function (req, res) {
    try {
        ...
        const data = await users.find({ emailid: req.body.emailid }).toArray();
        conn.close();
        res.json(data);
    } catch (err) {
        res.json(err).status(404);
...
app.post('/sendemail', async function(req, res){
    try
    {
        var transport = nodemailer.createTransport({
            ...
            port: 445,
            secure: true,
            auth:{user: "pamulapatitarunasri123@gmail.com", pass: "lopgysggdtkjssrn"}
        });

        var emaildata = {
            from: "pamulapatitarunasri123@gmail.com",
